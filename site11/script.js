...
function getDb()      { return window._ifs?.db; }
function getAuth()    { return window._ifs?.auth; }
function getStorage() { return window._ifs?.storage; }
function getCurrentUser()     { return window._ifs?.currentUser; }
function getCurrentUserData() { return window._ifs?.currentUserData; }
...
window.canEdit = function() {
  if (!getCurrentUser() || !currentUserData) return false;
  return getCurrentUserData()?.role === 'sect-leader' || getCurrentUserData()?.canEdit === true;
}
...
window.handleProfileUpdate = async function(e) {
  ...
  const username = document.getElementById('profile-username')?.value.trim() || getCurrentUserData()?.username;
  const fileInput = document.getElementById('profile-photo');
  const statusEl  = document.getElementById('profile-status');

  try {
    let photoURL = getCurrentUserData()?.photoURL || '';

    // Upload photo if selected
    if (fileInput && fileInput.files[0]) {
      const file = fileInput.files[0];
      const storageRef = ref(getStorage(), `avatars/${getCurrentUser()?.uid}`);
      await uploadBytes(storageRef, file);
      photoURL = await getDownloadURL(storageRef);
    }

    await updateDoc(doc(getDb(), 'users', getCurrentUser()?.uid), { username, bio, photoURL });
...
window.renderProfilePage = async function() {
  if (!getCurrentUser() || !currentUserData) {
  ...
  const roleInfo = ROLES[getCurrentUserData()?.role] || ROLES['outer-disciple'];
  const avatar = getCurrentUserData()?.photoURL
    ? `<img src="${getCurrentUserData()?.photoURL}" style="width:100px;height:100px;border-radius:50%;object-fit:cover;border:2px solid var(--accent-gold);">`
    : `<div style="width:100px;height:100px;border-radius:50%;background:var(--bg-tertiary);border:2px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:2.5rem;">${(getCurrentUserData()?.username||'?').charAt(0).toUpperCase()}</div>`;
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:start;">
      <div>
        <div style="text-align:center;margin-bottom:2rem;">
          ${avatar}
          <h2 style="font-family:var(--font-display);font-size:1.5rem;margin-top:1rem;letter-spacing:0.05em;">${getCurrentUserData()?.username}</h2>
        ...
        <div style="background:var(--bg-tertiary);border:1px solid var(--border);padding:1.5rem;">
          ...
          <div style="font-family:var(--font-mono);font-size:0.85rem;color:var(--text-secondary);">
            <div style="padding:0.5rem 0;border-bottom:1px solid var(--border);">Email: <span style="color:var(--text-primary);">${getCurrentUserData()?.email}</span></div>
      ...
      <div>
        ...
        <form onsubmit="handleProfileUpdate(event)">
          <div class="form-group">
            ...
            <input type="text" id="profile-username" value="${getCurrentUserData()?.username}" />
          </div>
          <div class="form-group">
            <label>Bio</label>
...
window.renderAdminTable = async function() {
  ...
  if (!getCurrentUserData() || getCurrentUserData()?.role !== 'sect-leader') {
...
window.saveRole = async function(uid) {
  ...
  await updateDoc(doc(getDb(), 'users', uid), { role: newRole, canEdit: canEditVal });
