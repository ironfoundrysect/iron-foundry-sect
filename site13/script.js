...
window.renderAdminTable = async function() {
  ...
  if (!tbody) return;
  try { await waitForIfs(); } catch(e) { return; }

  const ud = window._ifs?.currentUserData;
  if (!ud || ud.role !== 'sect-leader') {
    tbody.innerHTML = '<tr><td colspan="5" style="color:var(--text-muted);font-family:var(--font-mono);font-size:0.8rem;">Log in as Sect Leader to access this panel.</td></tr>';
    return;
  }
