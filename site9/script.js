...
function updateNavAuth() {
  ...
  if (currentUser && currentUserData) {
    // Logged in — force update every element
    ...
    const initial = (currentUserData.username || currentUserData.email || '?').charAt(0).toUpperCase();
    const avatar = currentUserData.photoURL
      ...
      : `<div style="width:28px;height:28px;border-radius:50%;background:var(--bg-tertiary);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:0.8rem;color:var(--accent-gold);">${initial}</div>`;

    if (userDisp) {
      userDisp.innerHTML = `
        <a href="profile.html" style="display:flex;align-items:center;gap:0.5rem;text-decoration:none;">
          ${avatar}
          <span style="color:var(--text-secondary);font-family:var(--font-mono);font-size:0.75rem;">${currentUserData.username || currentUserData.email}</span>
          <span class="role-badge ${roleInfo.class}">${roleInfo.label}</span>
        </a>
      `;
    }
    if (adminLink) {
      adminLink.style.display = currentUserData.role === 'sect-leader' ? 'inline' : 'none';
    }
  } else {
    // Logged out — show login/join, hide everything else
...
async function handleRegister(e) {
  ...
  try {
    ...
    closeAllModals();
    // onAuthStateChanged will fire and update the nav automatically
    e.target.reset();
  } catch (error) {
